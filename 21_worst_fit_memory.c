...
int worst_fit_allocation(char *process_name, int size) {
    int worst_idx = -1;
    int worst_size = -1;
    
    printf("\nAllocating %d bytes for %s using Worst Fit algorithm:\n", size, process_name);
    
    // Find the worst fit (largest free block that fits)
    for (int i = 0; i < block_count; i++) {
        if (!memory[i].is_allocated && memory[i].size >= size) {
            if (memory[i].size > worst_size) {
    ...
    printf("Worst fit block found: Block %d (size: %d)\n", worst_idx, memory[worst_idx].size);
    
    // Split the block if necessary
    if (memory[worst_idx].size > size) {
        ...
        // Shift blocks
        for (int i = block_count; i > worst_idx + 1; i--) {
            memory[i] = memory[i - 1];
        }
        
        // Create free block with remaining space
        memory[worst_idx + 1].block_id = worst_idx + 1;
        memory[worst_idx + 1].start_address = memory[worst_idx].start_address + size;
        memory[worst_idx + 1].size = memory[worst_idx].size - size;
        memory[worst_idx + 1].is_allocated = 0;
        strcpy(memory[worst_idx + 1].process_name, "Free");
    ...
    strcpy(memory[worst_idx].process_name, process_name);
    
    printf("Successfully allocated %d bytes to %s at address %d\n",
           size, process_name, memory[worst_idx].start_address);
...
int main() {
    ...
    printf("====================================\n");
    printf("Worst Fit: Allocate from the LARGEST available block\n\n");
    ...
    worst_fit_allocation("Process_A", 150);
    display_memory();
    get_memory_stats();
    
    worst_fit_allocation("Process_B", 200);
    display_memory();
    get_memory_stats();
    
    worst_fit_allocation("Process_C", 100);
    display_memory();
    get_memory_stats();
    
    worst_fit_allocation("Process_D", 250);
    display_memory();
    get_memory_stats();
    ...
    worst_fit_allocation("Process_E", 150);
    display_memory();
    get_memory_stats();
    
    printf("\n========== Analysis ==========\n");
    printf("Worst Fit advantages:\n");
    printf("- Leaves larger free blocks for future allocations\n");
    printf("Worst Fit disadvantages:\n");
    printf("- Slower allocation (searches all blocks)\n");
    printf("- May waste memory by not utilizing small gaps\n");
