...
int first_fit_allocation(char *process_name, int size) {
    int first_idx = -1;
    
    printf("\nAllocating %d bytes for %s using First Fit algorithm:\n", size, process_name);
    ...
    printf("First fit block found: Block %d (size: %d)\n", first_idx, memory[first_idx].size);
    
    // Split the block if necessary
    if (memory[first_idx].size > size) {
        ...
        // Shift blocks
        for (int i = block_count; i > first_idx + 1; i--) {
            memory[i] = memory[i - 1];
        }
        
        // Create free block with remaining space
        memory[first_idx + 1].block_id = first_idx + 1;
        memory[first_idx + 1].start_address = memory[first_idx].start_address + size;
        memory[first_idx + 1].size = memory[first_idx].size - size;
        memory[first_idx + 1].is_allocated = 0;
        strcpy(memory[first_idx + 1].process_name, "Free");
        
        block_count++;
    }
    
    // Allocate the block
    memory[first_idx].size = size;
    memory[first_idx].is_allocated = 1;
    strcpy(memory[first_idx].process_name, process_name);
    
    printf("Successfully allocated %d bytes to %s at address %d\n",
           size, process_name, memory[first_idx].start_address);
...
int main() {
    ...
    printf("====================================\n");
    printf("First Fit: Allocate from the FIRST available block that fits\n\n");
    ...
    first_fit_allocation("Process_A", 150);
    display_memory();
    get_memory_stats();
    
    first_fit_allocation("Process_B", 200);
    display_memory();
    get_memory_stats();
    
    first_fit_allocation("Process_C", 100);
    display_memory();
    get_memory_stats();
    
    first_fit_allocation("Process_D", 250);
    display_memory();
    get_memory_stats();
    ...
    first_fit_allocation("Process_E", 150);
    display_memory();
    get_memory_stats();
    
    printf("\n========== Analysis ==========\n");
    printf("First Fit advantages:\n");
    printf("- Fastest allocation algorithm\n");
    printf("- Simple implementation\n");
    printf("- Good overall performance\n");
    printf("First Fit disadvantages:\n");
    printf("- May waste some memory\n");
    printf("- Can lead to fragmentation over time\n");
